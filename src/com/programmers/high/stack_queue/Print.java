...
public class Print {
    ...
    */
    public int solution(int[] priorities, int location) {
        ...
        //우선순위 높은 Queue
        Queue<Integer> highQueue = Arrays.stream(priorities).boxed().sorted(Collections.reverseOrder()).collect(Collectors.toCollection(LinkedList::new));
        while(! queue.isEmpty()){
            int waitPrintPriority = queue.poll();
            if( waitPrintPriority == highQueue.peek()){
                answer++;
                highQueue.poll();
                if(location  == 0){
                    break;
